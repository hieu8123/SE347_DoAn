...
import { actionGetAllProductsShop } from "../../redux/actions/product";
...
const ProductDetails = ({ data }) => {
  ...
  useEffect(() => {
    dispatch(actionGetAllProductsShop(data && data?.shop._id));
  ...
  const removeFromWishlistHandler = (data) => {
    setClick(!click);
    dispatch(actionRemoveFromWishlist(data));
  };

  const addToWishlistHandler = (data) => {
    setClick(!click);
    dispatch(actionAddToWishlist(data));
  };
  ...
  const addToCartHandler = (id) => {
    ...
    if (isItemExists) {
      ...
      if (data.stock < 1) {
        ...
        dispatch(actionAddToCart(cartData));
